...
public class FlowerControllerUnitTest {
    ...
    public void isFlower_Test() {
        when(flowerService.analize(eq("Ageratum"))).thenReturn("flower");
        String flower = flowerController.isAFlower("Ageratum");
        assertEquals("flower", flower);
    }

    @Test
    public void isBigFlower_Test() {
        when(flowerService.isABigFlower(eq("Carnation"), anyInt())).thenReturn(true);
        Flower flower = new Flower("Carnation", 25);
